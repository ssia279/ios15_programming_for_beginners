...
class Burger: CalorieCount {
  ...
  func description() -> String {
    return "This burger has \(self.calories) calories"
  }
...
struct Fries: CalorieCount {
  ...
  func description() -> String {
    return "These fries have \(self.calories) calories"
  }
}
enum Sauce {
  case chili
...
extension Sauce: CalorieCount {
  ...
  func description() -> String {
    return "This sauce has \(self.calories) calories"
  }
}
...
do {
  try print(checkWebsite(siteUp: siteStatus))
} catch {
  print(error)
}
